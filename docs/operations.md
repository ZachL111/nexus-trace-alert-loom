...
`nexus-trace-alert-loom` is centered on observability work in Scala. The technical target is: Package a Scala local lab for alert analysis with capacity fixtures, allocation and spill reports, and documented operating limits.
...
The score is deterministic. The current threshold is 154. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
